...
/// Turns the usage API JSON into a `ClaudeUsageSnapshot`.
///
...
public enum ClaudeUsageDecoder {
    ...
    public static func decode(_ data: Data, fetchedAt: Date) throws -> ClaudeUsageSnapshot {
        ...
        for limit in response.limits ?? [] {
            ...
            let bucketKind: ClaudeUsageBucket.Kind
            switch kind {
            ...
            buckets.append(ClaudeUsageBucket(kind: bucketKind, percent: Int(percent.rounded()), resetsAt: parseDate(limit.resets_at)))
        }

        if buckets.isEmpty {
            if let bucket = response.five_hour, let utilization = bucket.utilization {
                buckets.append(ClaudeUsageBucket(kind: .session, percent: Int(utilization.rounded()), resetsAt: parseDate(bucket.resets_at)))
            }
            if let bucket = response.seven_day, let utilization = bucket.utilization {
                buckets.append(ClaudeUsageBucket(kind: .weeklyAll, percent: Int(utilization.rounded()), resetsAt: parseDate(bucket.resets_at)))
            }
        }

        guard !buckets.isEmpty else { throw UsageError.decoding }
        return ClaudeUsageSnapshot(buckets: sortedForDisplay(buckets), fetchedAt: fetchedAt)
    }

    /// session, weekly_all, scoped models (API order), then unknown kinds (API order).
    public static func sortedForDisplay(_ buckets: [ClaudeUsageBucket]) -> [ClaudeUsageBucket] {
        func rank(_ kind: ClaudeUsageBucket.Kind) -> Int {
            switch kind {
